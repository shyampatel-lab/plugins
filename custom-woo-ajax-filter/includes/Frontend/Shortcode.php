...
class Shortcode {
	...
	public function render( array $atts ): string {
		...
		<?php if($source && 'price'!==$source && in_array($type,array('checkbox','radio','dropdown'),true)): $terms=get_terms(array('taxonomy'=>$source,'hide_empty'=>true)); if(!is_wp_error($terms)): if('dropdown'===$type): ?><select name="<?php echo esc_attr($source);?>"><option value="">Any</option><?php foreach($terms as $t):?><option value="<?php echo esc_attr($t->slug);?>"><?php echo esc_html($t->name);?></option><?php endforeach;?></select><?php else: foreach($terms as $t): $img_id=(int)get_term_meta($t->term_id,'thumbnail_id',true); $img=$img_id?wp_get_attachment_image($img_id,'thumbnail',false,array('class'=>'cwaf-term-img')):'';?><label class="cwaf-term-label"><?php echo $img; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?><input type="<?php echo esc_attr('radio'===$type?'radio':'checkbox');?>" name="<?php echo esc_attr($source.('checkbox'===$type?'[]':''));?>" value="<?php echo esc_attr($t->slug);?>"> <span><?php echo esc_html($t->name);?></span></label><?php endforeach; endif; endif; endif; ?>
